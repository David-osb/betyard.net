...
class NFLMLModel:
    ...
    def _initialize_models(self):
        ...
        try:
            ...
            # Models are in the parent backend directory
            model_dir = os.path.join(os.path.dirname(__file__), '..')
            multiposition_path = os.path.join(model_dir, 'modern_multiposition_models.pkl')
        ...
        for position in positions:
            # Models are in the parent backend directory
            model_dir = os.path.join(os.path.dirname(__file__), '..')
            model_path = os.path.join(model_dir, f'{position.lower()}_model.pkl')
    ...
    def _create_and_train_model(self, position: str):
        ...
        # Save model to parent backend directory
        model_dir = os.path.join(os.path.dirname(__file__), '..')
        model_path = os.path.join(model_dir, f'{position.lower()}_model.pkl')
...
def get_team_abbr_from_name(team_name):
    """Convert full team name or abbreviation to standard abbreviation"""
    # Handle abbreviations directly
    abbr_map = {
        'LAL': 'LAL', 'GSW': 'GSW', 'BOS': 'BOS', 'MIA': 'MIA',
        'CHI': 'CHI', 'NYK': 'NYK', 'LAC': 'LAC', 'DEN': 'DEN',
        'PHX': 'PHX', 'MIL': 'MIL', 'PHI': 'PHI', 'BKN': 'BKN',
        'ATL': 'ATL', 'CLE': 'CLE', 'DAL': 'DAL', 'DET': 'DET',
        'HOU': 'HOU', 'IND': 'IND', 'MEM': 'MEM', 'MIN': 'MIN',
        'NOP': 'NOP', 'OKC': 'OKC', 'ORL': 'ORL', 'POR': 'POR',
    ...
    name_map = {
        ...
        'LA Clippers': 'LAC', 'Denver Nuggets': 'DEN',
        'Phoenix Suns': 'PHX', 'Milwaukee Bucks': 'MIL',
        'Philadelphia 76ers': 'PHI', 'Brooklyn Nets': 'BKN',
        'Atlanta Hawks': 'ATL', 'Cleveland Cavaliers': 'CLE',
        ...
        'Washington Wizards': 'WAS', 'Charlotte Hornets': 'CHA'
    }
...
def get_nba_team_players(team_identifier):
    ...
    try:
        ...
        logger.error(f"🏀 Error fetching NBA team players: {str(e)}")
        return jsonify({'success': False, 'error': str(e)}), 500
...
if __name__ == '__main__':
    ...
    logger.info("  GET  /api/nba/games/today - NBA games today")
    logger.info("  GET  /api/nba/teams - All NBA teams")
    logger.info("  GET  /players/nba/team/<team> - NBA team roster")
